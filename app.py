...
# Custom CSS Injection for Premium UI (As seen in image_84b8c2.png)
st.markdown("""
    <style>
...
def get_gspread_client():
    ...
    # Extract keys safely regardless of nesting level
...
def read_gsheet_data(spreadsheet_url_or_id, worksheet_name="Sheet1"):
    try:
...
def update_gsheet_data(spreadsheet_url_or_id, dataframe, worksheet_name="Sheet1"):
    client = get_gspread_client()
    sheet_id = spreadsheet_url_or_id
    if "docs.google.com/spreadsheets" in spreadsheet_url_or_id:
        match = re.search(r"/d/([a-zA-Z0-9-_]+)", spreadsheet_url_or_id)
...
# Locate config contexts safely
spreadsheet_target = ""
if "connections" in st.secrets and "gsheets" in st.secrets["connections"]:
    spreadsheet_target = st.secrets["connections"]["gsheets"].get("spreadsheet", st.secrets["connections"]["gsheets"].get("url", ""))
else:
    spreadsheet_target = st.secrets.get("spreadsheet", st.secrets.get("url", ""))

# Load current active sheet dataframe
...
with tab_dash:
    if filtered_df.empty:
        ...
        with col3:
            pos_pct = round((len(filtered_df[filtered_df['Sentiment'] == 'Positive']) / len(filtered_df)) * 100) if len(filtered_df) > 0 else 0
