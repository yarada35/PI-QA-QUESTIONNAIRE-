...
# ==========================================
# 3. EXPLICIT CONNECTION CONFIGURATION (Fixed ValueError)
# ==========================================
# 1. Safely initialize secrets data structures
if "connections" not in st.secrets._secrets:
    st.secrets._secrets["connections"] = {}

# 2. Paste your literal configuration variables exactly as generated by Google
GOOGLE_CREDENTIALS_DATA = {
    "type": "service_account",
    "project_id": "YOUR_PROJECT_ID",
    "private_key_id": "YOUR_PRIVATE_KEY_ID",
    # TIP: Ensure the private key retains its full layout exactly as provided in the JSON file.
...
# 3. Strip any weird leading/trailing whitespace safely without corrupting internal formatting
if "private_key" in GOOGLE_CREDENTIALS_DATA:
    # Resolves internal newlines without altering block layout
    GOOGLE_CREDENTIALS_DATA["private_key"] = GOOGLE_CREDENTIALS_DATA["private_key"].strip().replace("\\n", "\n")

# 4. Bind values cleanly into Streamlit's secrets engine mapping
st.secrets._secrets["connections"]["gsheets"] = {
    "spreadsheet": TARGET_SPREADSHEET_URL,
    **GOOGLE_CREDENTIALS_DATA
}

# 5. Connect cleanly using compliant signatures
